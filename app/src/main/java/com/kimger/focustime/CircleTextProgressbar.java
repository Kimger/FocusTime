...
public class CircleTextProgressbar extends TextView {
    ...
    private void initialize(Context context, AttributeSet attributeSet) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attributeSet,
                R.styleable.CircleTextProgressbar);
        if (typedArray.hasValue(R.styleable.CircleTextProgressbar_in_circle_color))
            inCircleColors =
                    typedArray.getColorStateList(R.styleable.CircleTextProgressbar_in_circle_color);
        else
    ...
    private void validateCircleColor() {
        int circleColorTemp = inCircleColors.getColorForState(getDrawableState(),
                Color.TRANSPARENT);
     ...
     */
    public void setCountdownProgressListener(int what,
                                             OnCountdownProgressListener mCountdownProgressListener) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(bounds.centerX(), bounds.centerY(), outerRadius - outLineWidth / 2,
                mPaint);
        ...
        mArcRect.set(bounds.left + deleteWidth / 3, bounds.top + deleteWidth / 3,
                bounds.right - deleteWidth / 3, bounds.bottom - deleteWidth / 3);
        float baifen = (float) progress / (float) maxProgress * 100;
        canvas.drawArc(mArcRect, 270, ((float) progress / (float) maxProgress) * 360f, false,
                mPaint);
    }
        ...
        public void run() {
            ...
            if (progress >= 0 && progress <= maxProgress) {
                if (mCountdownProgressListener != null)
                    mCountdownProgressListener.onProgress(listenerWhat, progress, --lastTime);
                invalidate();
                postDelayed(progressChangeTask, 1000);
            } else {
    ...
    public interface OnCountdownProgressListener {
         ...
         * @param progress 进度值。
         */
        void onProgress(int what, int progress, long time);
